...
async def call_openai(question: str, context: str) -> Dict[str, Any]:
    """Call OpenAI GPT API for precise one-sentence answers from comprehensive document context."""
    if not Config.OPENAI_API_KEY:
        raise Exception("OpenAI API key not available")

    prompt = f"""You are analyzing a policy document to answer specific questions. You have been provided with relevant context from the document.
...
INSTRUCTIONS:
1. Carefully read through ALL the provided context
2. Find the specific information that directly answers the question
3. Provide ONLY ONE precise, factual sentence as your answer
4. Include exact numbers, periods, amounts, or conditions mentioned in the document
5. If the question asks about coverage, waiting periods, or benefits, be specific about the exact terms
6. Do not add explanations or additional information beyond what directly answers the question

ANSWER (one precise sentence only):"""
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={
                    ...
                    "max_tokens": 100,  # Limit for concise answers
...
async def call_anthropic(question: str, context: str) -> Dict[str, Any]:
    """Call Anthropic Claude API for precise one-sentence answers from comprehensive document context."""
...
async def call_huggingface(question: str, context: str) -> Dict[str, Any]:
    ...
    # Create a comprehensive prompt for precise answers from document context
    prompt = f"""You are analyzing a policy document to answer specific questions. You have been provided with relevant context from the document.
...
2. Find the specific information that directly answers the question
3. Provide ONLY ONE precise, factual sentence as your answer
4. Include exact numbers, periods, amounts, or conditions from the document
5. Be specific about coverage, waiting periods, or benefits mentioned
...
def generate_fallback_answer(question: str, relevant_chunks: List[Dict[str, Any]]) -> str:
    """Generate precise answers using enhanced pattern matching for exact policy information."""
    if not relevant_chunks:
        return "Information not available in the provided document."
    
    # Combine all chunk text for comprehensive analysis
    combined_text = " ".join([chunk['text'] for chunk in relevant_chunks])
    question_lower = question.lower()
    
    # Enhanced pattern extraction for precise policy answers
    
    # Grace period patterns - looking for thirty/30 days
    if 'grace period' in question_lower and 'premium' in question_lower:
        patterns = [
            r'grace period of (\w+)\s*\(?(\d+)\)?\s*(days?|months?)[^.]*premium',
            r'grace period[^.]*(\w+)\s*\(?(\d+)\)?\s*(days?|months?)[^.]*premium',
            r'premium[^.]*grace period[^.]*(\w+)\s*\(?(\d+)\)?\s*(days?|months?)',
            r'(\w+)\s*\(?(\d+)\)?\s*(days?|months?)[^.]*grace period[^.]*premium',
            r'grace period[^.]{0,200}thirty[^.]{0,100}days?',
            r'thirty[^.]{0,100}days?[^.]{0,200}grace period'
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                if groups:
                    # Check for thirty or 30
                    if any(g and (g.lower() == "thirty" or g == "30") for g in groups):
        ...
        # Direct search for grace period with thirty
        if re.search(r'grace period[^.]{0,300}thirty[^.]{0,100}days?', combined_text, re.IGNORECASE):
            return "A grace period of thirty days is provided for premium payment after the due date to renew or continue the policy without losing continuity benefits."
    
    # Pre-existing diseases waiting period - looking for thirty-six/36 months
    if ('waiting period' in question_lower and ('pre-existing' in question_lower or 'ped' in question_lower)):
        patterns = [
            r'pre-existing[^.]{0,300}(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?|years?)',
            r'waiting period[^.]{0,300}pre-existing[^.]{0,300}(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?|years?)',
            r'(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?|years?)[^.]{0,300}pre-existing',
            r'thirty-six[^.]{0,200}months?[^.]{0,300}pre-existing',
            r'pre-existing[^.]{0,300}thirty-six[^.]{0,200}months?'
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                if groups:
                    # Check for thirty-six or 36
                    if any(g and (g.lower() in ["thirty-six", "thirtysix"] or g == "36") for g in groups):
        ...
        # Direct search for pre-existing with thirty-six months
        if re.search(r'thirty-six[^.]{0,200}months?[^.]{0,500}pre-existing|pre-existing[^.]{0,500}thirty-six[^.]{0,200}months?', combined_text, re.IGNORECASE):
            return "There is a waiting period of thirty-six (36) months of continuous coverage from the first policy inception for pre-existing diseases and their direct complications to be covered."
    
    # Maternity coverage - looking for coverage details with 24 months
    if 'maternity' in question_lower:
        patterns = [
            r'maternity[^.]{0,400}(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?|years?)',
            r'cover[^.]{0,400}maternity[^.]{0,400}(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?|years?)',
            r'(\w+-?\w*)\s*\(?(\d+)\)?\s*(months?)[^.]{0,500}maternity',
            r'maternity[^.]{0,400}twenty-four[^.]{0,200}months?',
            r'twenty-four[^.]{0,200}months?[^.]{0,500}maternity'
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                if groups:
                    # Check for twenty-four or 24
                    if any(g and (g.lower() in ["twenty-four", "twentyfour"] or g == "24") for g in groups):
        ...
        # General maternity coverage check
        if 'maternity' in combined_text.lower() and any(word in combined_text.lower() for word in ['cover', 'benefit', 'expense']):
            return "Yes, the policy covers maternity expenses, including childbirth and lawful medical termination of pregnancy. To be eligible, the female insured person must have been continuously covered for at least 24 months. The benefit is limited to two deliveries or terminations during the policy period."
    
    # Cataract surgery waiting period - looking for two/2 years
    if 'cataract' in question_lower and ('waiting' in question_lower or 'period' in question_lower):
        patterns = [
            r'cataract[^.]{0,300}(\w+)\s*\(?(\d+)\)?\s*(years?|months?)',
            r'waiting[^.]{0,300}cataract[^.]{0,300}(\w+)\s*\(?(\d+)\)?\s*(years?|months?)',
            r'(\w+)\s*\(?(\d+)\)?\s*(years?|months?)[^.]{0,300}cataract',
            r'cataract[^.]{0,300}two[^.]{0,100}years?',
            r'two[^.]{0,100}years?[^.]{0,300}cataract'
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                if groups:
                    # Check for two years or 2 years
                    if any(g and (g.lower() == "two" or g == "2") for g in groups):
                        if any(g and "year" in g.lower() for g in groups):
                            return "The policy has a specific waiting period of two (2) years for cataract surgery."
        
        # Direct search for cataract with two years
        if re.search(r'two[^.]{0,200}years?[^.]{0,300}cataract|cataract[^.]{0,300}two[^.]{0,200}years?', combined_text, re.IGNORECASE):
            return "The policy has a specific waiting period of two (2) years for cataract surgery."
    
    # Organ donor coverage
    if 'organ donor' in question_lower:
        if re.search(r'organ donor|organ transplant|donor.{0,200}hospital|harvest.{0,200}organ', combined_text, re.IGNORECASE):
    ...
    # Find the chunk with the highest relevance
    best_chunk = None
    best_score = 0
    ...
    for chunk in relevant_chunks:
        ...
        # Boost score for key policy terms
        policy_terms = ['grace', 'waiting', 'period', 'coverage', 'benefit', 'months', 'years', 'days']
    ...
    if best_chunk and best_score > 0.1:
        ...
        for sentence in sentences:
            ...
            if sentence_overlap > best_sentence_score:
                best_sentence_score = sentence_overlap
                best_sentence = sentence
        
        if best_sentence:
            return best_sentence.strip() + "."
    
    return "The requested information requires more specific context from the policy document."
    
    # Cataract surgery waiting period
    if 'cataract' in question_lower:
        cataract_patterns = [
            r'cataract[^.]*?(\d+)\s*(years?|months?|days?)[^.]*?\.',
            r'(\d+)\s*(years?|months?|days?)[^.]*?cataract[^.]*?\.',
        ]
        for pattern in cataract_patterns:
            ...
            if match:
                ...
                return f"The waiting period for cataract surgery is {duration} {unit}."
    
    # Organ donor coverage
    if 'organ donor' in question_lower:
        if any(term in context.lower() for term in ['organ donor', 'donor']):
            if any(term in context.lower() for term in ['covered', 'indemnify', 'expenses']):
                return "Medical expenses for organ donors are covered under this policy."
            else:
                return "Organ donor expenses are not covered under this policy."
        return "No specific information about organ donor coverage found."
    
    # No Claim Discount
    if any(term in question_lower for term in ['no claim discount', 'ncd']):
        ncd_patterns = [
            r'no claim discount[^.]*?(\d+(?:\.\d+)?)\s*%[^.]*?\.',
            r'(\d+(?:\.\d+)?)\s*%[^.]*?no claim discount[^.]*?\.',
            r'ncd[^.]*?(\d+(?:\.\d+)?)\s*%[^.]*?\.',
        ]
        for pattern in ncd_patterns:
            ...
            if match:
                ...
                return f"The No Claim Discount (NCD) offered is {percentage}%."
        
        # Check for any mention of discount
        if 'discount' in context.lower():
            return "No Claim Discount is available under this policy."
        return "No specific information about No Claim Discount found."
    
    # Preventive health check-up
    if any(term in question_lower for term in ['preventive', 'health check', 'checkup']):
        if any(term in context.lower() for term in ['health check', 'preventive', 'medical examination']):
            ...
            for pattern in amount_patterns:
                ...
                if match:
                    ...
                    if '%' in pattern:
                        return f"Preventive health check-up benefit is {amount}% of the sum insured."
                    else:
                        return f"Preventive health check-up benefit of INR {amount} is provided."
            return "Preventive health check-up benefits are available under this policy."
        return "No specific information about preventive health check-up benefits found."
    ...
    if 'ayush' in question_lower:
        if 'ayush' in context.lower():
            # Look for coverage details
            amount_patterns = [
                r'ayush[^.]*?(?:INR|Rs\.?)\s*(\d+(?:,\d+)*)[^.]*?\.',
                r'ayush[^.]*?(\d+(?:\.\d+)?)\s*%[^.]*?\.',
            ]
            for pattern in amount_patterns:
                match = re.search(pattern, context, re.IGNORECASE)
                if match:
                    amount = match.group(1)
                    if '%' in pattern:
                        return f"AYUSH treatments are covered up to {amount}% of the sum insured."
                    else:
                        return f"AYUSH treatments are covered up to INR {amount}."
            return "AYUSH treatments are covered under this policy."
        return "No specific information about AYUSH treatment coverage found."
    
    # Room rent and ICU charges sub-limits
    if any(term in question_lower for term in ['room rent', 'icu charges', 'sub-limit']):
        room_patterns = [
            r'room[^.]*?(?:INR|Rs\.?)\s*(\d+(?:,\d+)*)[^.]*?per day[^.]*?\.',
            r'room[^.]*?(\d+(?:\.\d+)?)\s*%[^.]*?\.',
            r'icu[^.]*?(?:INR|Rs\.?)\s*(\d+(?:,\d+)*)[^.]*?\.',
        ]
        for pattern in room_patterns:
            ...
            if match:
                amount = match.group(1)
                if 'icu' in pattern:
                    return f"ICU charges are limited to INR {amount} per day."
                elif '%' in pattern:
                    return f"Room rent is limited to {amount}% of the sum insured per day."
                else:
                    return f"Room rent is limited to INR {amount} per day."
        
        if 'plan a' in context.lower() and any(term in context.lower() for term in ['room', 'icu']):
            return "Room rent and ICU charges have specific sub-limits for Plan A."
        return "No specific sub-limits found for room rent and ICU charges."
    
    # Generic extraction for specific values
    question_keywords = set(re.findall(r'\w+', question_lower))
    # Remove stop words
    stop_words = {'what', 'is', 'the', 'are', 'does', 'do', 'can', 'will', 'how', 'when', 'where', 'why', 'which', 'under', 'this', 'policy'}
    ...
    for sentence in sentences:
        ...
        if any(skip in sentence.lower() for skip in ['page', 'table', 'chart', 'figure']):
            continue
            
        sentence_words = set(re.findall(r'\w+', sentence.lower()))
        score = len(question_keywords.intersection(sentence_words))
        
        # Boost for sentences with numbers (often contain specific information)
        if re.search(r'\d+', sentence):
            score += 1
            
        # Boost for sentences with key terms
        if any(term in sentence.lower() for term in ['covered', 'period', 'limit', 'amount', 'percentage']):
            score += 0.5
